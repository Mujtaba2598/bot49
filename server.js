...
// ==================== API KEY MANAGEMENT - COMPLETELY FIXED ====================
...
app.post('/api/set-api-keys', authenticate, async (req, res) => {
    ...
    if (!apiKey || !secretKey) {
        return res.status(400).json({ success: false, message: 'Both API key and secret key are required' });
    }
    
    const cleanApi = cleanKey(apiKey);
    const cleanSecret = cleanKey(secretKey);
    
    if (!cleanApi || !cleanSecret) {
        return res.status(400).json({ success: false, message: 'Invalid API key format. Remove spaces and line breaks.' });
    }
    ...
    try {
        ...
        if (!accountData.canTrade) {
            return res.status(401).json({ success: false, message: 'Please enable "Spot & Margin Trading" in your Binance API settings.' });
        }
        ...
        res.json({ success: true, message: 'API keys saved and verified successfully!' });
        ...
        if (error.response?.data?.code === -2015) {
            errorMessage += 'API key format invalid or permissions missing. Enable "Spot & Margin Trading".';
        } else if (error.response?.data?.code === -1021) {
            errorMessage += 'Timestamp error. Server time sync issue.';
        } else if (error.response?.data?.code === -1022) {
            errorMessage += 'Signature error. Check that your Secret Key is correct (no missing characters).';
        } else if (error.response?.status === 401) {
            errorMessage += 'Invalid API key or secret key. Check for typos or missing characters.';
        } else {
...
class BinanceAPI {
    ...
    static generateSignature(queryString, secret) {
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ Binance API signature: STANDARD (no percent-encoding)`);
    console.log(`✅ Time sync: uses Binance server time\n`);
});
